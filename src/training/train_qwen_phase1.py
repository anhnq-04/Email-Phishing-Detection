"""
train_qwen_phase1.py
====================
Phase 1: General bilingual phishing + evidence training
Final dataset policy: content-only Qwen V2, neutral [LINK_N], no domain_spoofing.
Current curriculum: EN:VIE ≈ 1:1, label-balanced.

Model: unsloth/Qwen3-14B (4-bit LoRA)
Data:  data/processed/qwen/phase1_train.jsonl
Eval:  data/processed/qwen/phase1_eval.jsonl

Hardware target: 1x NVIDIA A100 40GB
Run:
  python -m src.training.train_qwen_phase1
...
OUTPUT_DIR = _PROJECT_ROOT / "models" / "qwen" / "checkpoint_phase1"

TRAIN_PATH = str(DATA_DIR / "phase1_train.jsonl")
VAL_PATH   = str(DATA_DIR / "phase1_eval.jsonl")
...
# A100 supports TF32; useful for supported float32 matmul operations.
...
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    dtype=None,                 # A100 -> BF16 compute where supported.
    load_in_4bit=True,
    load_in_8bit=False,
...
def print_and_validate_trainable_parameters(model) -> None:
    ...
    if trainable == 0:
        raise RuntimeError("No trainable parameters found. LoRA adapter was not initialized correctly.")
...
def format_chat(example):
    ...
    # Critical for Qwen3: training and inference must both suppress <think> mode.
...
def build_sft_config() -> SFTConfig:
    ...
    # TRL renamed max_seq_length -> max_length in newer versions.
    if "max_length" in config_params:
        kwargs["max_length"] = MAX_SEQ_LENGTH
    elif "max_seq_length" in config_params:
        kwargs["max_seq_length"] = MAX_SEQ_LENGTH

    # TRL renamed evaluation_strategy -> eval_strategy in newer versions.
    ...
    # Avoid padding overhead while preserving complete examples.
...
def build_trainer() -> SFTTrainer:
    ...
    # Older Unsloth-patched SFTTrainer expects this argument directly.
...
if __name__ == "__main__":
    print("Training Phase 1 — Qwen V2, A100 40GB, MAX_SEQ_LENGTH=4096")
    ...
    # Final output path intentionally unchanged.
    model_output = str(_PROJECT_ROOT / "models" / "qwen" / "model_phase1")
    trainer.save_model(model_output)
    tokenizer.save_pretrained(model_output)

    print(f"Saved Phase 1 adapter/tokenizer -> {model_output}")
