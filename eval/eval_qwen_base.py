...
eval_qwen_base.py
=================
Evaluate the **base** Qwen3-14B model (no LoRA adapter) on the Phase 2 test set.

The model must be served via vLLM beforehand:
...
Usage:
    python -m eval.eval_qwen_base                          # defaults
    python -m eval.eval_qwen_base --max-samples 200        # quick smoke test
    python -m eval.eval_qwen_base --base-url http://host:8001/v1
"""

# from eval.eval_qwen_ft import ENABLE_THINKING
...
# Base model served WITHOUT --enable-lora → model name = the HF model name itself
DEFAULT_BASE_URL = "http://127.0.0.1:8001/v1"
DEFAULT_MODEL_NAME = "unsloth/qwen3-14b-unsloth-bnb-4bit"
...
def evaluate(
    ...
    log_path = os.path.join(log_dir, f"eval_qwen_base_{timestamp}.log")
...
def _evaluate_inner(
    ...
    details_path = os.path.join(output_dir, "results_qwen_base_details.jsonl")
    ...
    summary_path = os.path.join(output_dir, "results_qwen_base.json")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate Qwen3-14B BASE model via vLLM server"
    )
    parser.add_argument(
